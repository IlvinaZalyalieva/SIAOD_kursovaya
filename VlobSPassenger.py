...
class Bus:
    ...
    def move_to_next_stop(self):
        ...
        if self.time_to_next_stop > 0:
            ...
            if self.direction == 1:  # Движение вперед по верхнему маршруту
                #print('2', i)
                if self.current_stop_index < 18 - 1:
                    self.current_route = self.route
                else:
                    self.direction *= -1
                ...
                #print('3', i)
                if -17 < self.current_stop_index < 16 :
...
class Passenger:
    ...
    def board_passengers(self, bus):
        ...
        # Проверка индекса остановки
        if bus.current_stop_index < -18 or bus.current_stop_index >= len(self.schedule.stops):
            print(f"Bus {bus.bus_number}: Invalid stop index {bus.current_stop_index}!")
            return 0  # Возвращаем 0, если индекс некорректен
...
class BusScheduleApp(QMainWindow):
    def __init__(self, schedule):
        ...
        self.flag = 1
        self.info_list = []
        self.total_passengers_left = 0  # Добавляем переменную для хранения неуместившихся пассажиров
    ...
    def update_schedule(self):
        ...
        # Подсчитываем неуместившихся пассажиров
        for bus in self.schedule.buses:
            if not bus.on_parking:
                passengers_left = self.schedule.passenger_manager.board_passengers(bus)
                self.total_passengers_left += passengers_left  # Накапливаем неуместившихся пассажиров
...
# Создаем водителей
    ...
    # Устанавливаем скорость симуляции на 30x
    time_speed = 1
    ...
    # Если включена визуализация, запускаем приложение
    if visualize:
        app = QApplication(sys.argv)
        main_window = BusScheduleApp(schedule)
        main_window.show()
        main_window.time_speed = time_speed  # Устанавливаем скорость времени
        main_window.timer.setInterval(1000 // time_speed)  # Обновляем интервал таймера
        app.exec_()
    else:
        # Фоновая симуляция
        current_time = datetime(2024, 1, 1, 7, 0)
        total_passengers_left = 0

        while current_time < end_time:
            schedule.update(current_time)
            for bus in schedule.buses:
                if not bus.on_parking:
                    passengers_left = schedule.passenger_manager.board_passengers(bus)
                    total_passengers_left += passengers_left
            current_time += timedelta(minutes=1) / time_speed
        return total_passengers_left

    # Возвращаем общее количество неуместившихся пассажиров
    return main_window.total_passengers_left

if __name__ == "__main__":
    print(f"Общее кол-во невместившихся пассажиров {-main(visualize=True)}")